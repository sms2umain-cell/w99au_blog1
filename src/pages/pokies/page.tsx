...
import { PokiesHero } from './components/PokiesHero';
...
export default function PokiesPage() {
  ...
  useEffect(() => {
    updateSEO({
      title: 'W99AU Online Pokies Australia | High RTP Pokies & Winning Strategies 2025',
      description: 'Discover W99AU online pokies guides at W99AU Casino Australia. Expert reviews on high RTP pokies, progressive jackpots, Megaways games, free spins and winning strategies. Play 3000+ pokies with instant deposits.',
      keywords: 'W99AU online pokies, online pokies Australia, high RTP pokies, progressive jackpots, Megaways pokies, free spins, pokies strategies, Australian pokies, pokies blog, W99AU casino',
      canonical: 'https://w99aud.com/pokies',
      ogType: 'website',
      structuredData: generateBreadcrumbs([
        { name: 'Home', url: '/' },
        { name: 'Pokies Blog', url: '/pokies' }
      ])
    });
  }, []);
  ...
  const categories = [
    { name: 'All', count: pokiesArticles.length },
    ...Array.from(new Set(pokiesArticles.map(article => article.category))).map(category => ({
      name: category,
      count: pokiesArticles.filter(article => article.category === category).length
    }))
  ];
  ...
  const filteredArticles = selectedCategory === 'All' 
    ? pokiesArticles 
    : pokiesArticles.filter(article => article.category === selectedCategory);
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-gray-900 mb-4">Explore <strong className="text-yellow-600">W99AU Online Pokies</strong> Categories</h2>
            <p className="text-xl text-gray-600">Browse our comprehensive collection of <strong>online pokies Australia</strong> articles and guides</p>
          </div>
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <div className="text-sm text-gray-600 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <strong className="text-gray-900">{pokiesArticles.length}</strong>
                    </div>
                    <div className="flex items-center justify-between">
                ...
                {filteredArticles.map((article) => (
                  ...
                  >
                    <div className="relative h-48 overflow-hidden">
                      <img 
                        src={article.image} 
                        alt={`${article.title} - W99AU Online Pokies Guide`}
